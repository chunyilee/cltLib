...
public class SearchNet {
    ...
    public SearchNet() throws Exception {
        String ipbrocast = "";
        ...
        if (ipString.length != 4) {
            ...
            for (int i = 0; i < ipString.length-1; i++) {
                ipbrocast += ipString[i] + ".";
            }
        }
        int[] bounds = SearchNet.rangeFromCidr(ipbrocast + "255/24");  //orginal  "192.168.1.255/24"

        for (int i = bounds[0]; i <= bounds[1]; i++) {
            String address = InetRange.intToIp(i);
            InetAddress ip = InetAddress.getByName(address);

            if (ip.isReachable(500)) { // Try for one tenth of a second
